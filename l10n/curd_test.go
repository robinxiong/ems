...
func TestUpdate(t *testing.T) {
	...
	checkHasErr(t, dbGlobal.Create(&product).Error)
	sharedDB := dbGlobal.Model(&Product{}).Where("id = ? AND code = ?", product.ID, "Update") //返回的shareDB包含了Where查询条件

	product.Name = "中文名"
	checkHasErr(t, dbCN.Create(&product).Error) //创建一个中文记录
	...
	// 通过dbGlobal以及UpdateColumns方法来更新l10n:sync列
	...
	//查看当前数据库的相同id的pruoduct是否比 未更新前的product多2
